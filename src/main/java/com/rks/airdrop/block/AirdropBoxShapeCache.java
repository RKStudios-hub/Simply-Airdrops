...
final class AirdropBoxShapeCache {
    private static final ShapeBox[] MODEL_BOXES = {
            new ShapeBox(-5, 0, -6, 23, 3, 22),
            new ShapeBox(-5, 25, -6, 23, 27.5, 22),
            new ShapeBox(20, 3, 19, 23, 25, 22),
            new ShapeBox(8, -3, 21, 11, 30, 21.5, 0, 0, -45, 10, 14, 21),
            new ShapeBox(8, -3, -5.5, 11, 30, -5, 0, 0, 45, 10, 14, -5.5),
            new ShapeBox(-6.5, -3, 7.5, -3.5, 30, 8, 90, 45, 90, -4.5, 14, 7.5),
            new ShapeBox(20, -3, 7.5, 23, 30, 8, -90, 45, -90, 22, 14, 7.5),
            new ShapeBox(20, 3, -6, 23, 25, -3),
            new ShapeBox(-5, 3, -6, -2, 25, -3),
            new ShapeBox(-5, 3, 19, -2, 25, 22),
            new ShapeBox(-4, 1, -5, 22, 27, 21)
    };
    ...
    private AirdropBoxShapeCache() {
    }

    static VoxelShape getDetailedShape(Direction facing, int offsetX, int offsetY, int offsetZ) {
        return DETAILED_CACHE.computeIfAbsent(key(facing, offsetX, offsetY, offsetZ), unused -> buildDetailedShape(facing, offsetX, offsetY, offsetZ));
    }

    static VoxelShape getSolidShape(Direction facing, int offsetX, int offsetY, int offsetZ) {
        return SOLID_CACHE.computeIfAbsent(key(facing, offsetX, offsetY, offsetZ), unused -> buildSolidShape(facing, offsetX, offsetY, offsetZ));
    }
    ...
    private static VoxelShape buildDetailedShape(Direction facing, int offsetX, int offsetY, int offsetZ) {
        Bounds cell = new Bounds(offsetX * 16, offsetY * 16, offsetZ * 16, offsetX * 16 + 16, offsetY * 16 + 16, offsetZ * 16 + 16);
    ...
    private static final class ShapeBox {
        ...
        private ShapeBox(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
            this(minX, minY, minZ, maxX, maxY, maxZ, 0, 0, 0, 0, 0, 0);
        }
